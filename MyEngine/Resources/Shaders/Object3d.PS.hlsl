...
PixelShaderOutput main(VertexShaderOutput input)
{
    ...
    if (gMaterialData.enableLighting != 0)
    {
        ...
        if (gPointLightData.lightingType == 1 || gPointLightData.lightingType == 2)
        {
            ...
            float32_t factor = saturate(pow(-distance / gPointLightData.radius + 1.0f, gPointLightData.decay));
            //拡散反射
        ...
        if (gSpotLightData.lightingType == 1 || gSpotLightData.lightingType == 2)
        {
            ...
            float32_t attenuationFactor = saturate(pow(-distance / gSpotLightData.distance + 1.0f, gSpotLightData.decay));
            //拡散反射
